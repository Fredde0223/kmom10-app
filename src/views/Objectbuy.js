...
class Objectbuy extends React.Component {
  state = {
      selected: null,
      amount: null,
  };
  ...
  handleSubmit = async (event) => {
    event.preventDefault();
    await fetch('https://proj-api.frah19.me/objects/buy', {
        method: 'PUT',
        headers: {
    ...
    event.preventDefault();
    await fetch('https://proj-api.frah19.me/accounts/buy', {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json; charset=utf-8',
            'x-access-token': localStorage.token
...
export default withRouter(Objectbuy);
